...
(async()=>{const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 const keys=await page.evaluate(()=>ACH_BADGE_DEFS.filter(d=>!d.challenge).concat(KOTSU_IMG_DEFS).map(d=>d.key));
 ...
 await page.keyboard.press('Escape');await page.evaluate(()=>{closeAchPreview();show('home');selMode('no');});
 ...
 assert.deepEqual(errors,[]);console.log('PASS: countdown, late answer, retry, clear, normal record isolation, 1100-question endless run, navigation cleanup, all 42 lore images, phone dialogs');
