...
(async()=>{
 ...
 await page.evaluate(()=>{show('settings');document.querySelector('details.learning-settings').open=true;});
 const input=page.locator('details.learning-settings input[type=number]');await input.fill('4.1');await input.dispatchEvent('change');
