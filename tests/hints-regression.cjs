...
(async()=>{
 ...
 await page.waitForFunction(()=>document.querySelector('[data-cue="split-values"]').classList.contains('shown'),null,{timeout:5000});
 assert.equal(await page.locator('[data-cue="sum-answer"].shown').count(),0);
 await page.waitForTimeout(450);
 await page.locator('#hint-static-box').screenshot({path:'tests/sakura-genka-split.png'});
 await page.waitForFunction(()=>document.querySelector('[data-cue="subtract-fact"]').classList.contains('shown'),null,{timeout:10000});
 assert.equal(await page.locator('[data-cue="sum-answer"].shown').count(),0);
 await page.waitForTimeout(450);
 await page.locator('#hint-static-box').screenshot({path:'tests/sakura-genka-subtract.png'});
 await page.waitForFunction(()=>document.querySelector('.hunter-sakura-replay').textContent.includes('もういちど'),null,{timeout:15000});
 assert.equal(await page.locator('.hunter-sakura-message').innerText(),'のこりの1と3を足すよ。1たす3は4。');
 assert.equal(await page.locator('.hunter-sakura-svg .lesson-cue.shown').count(),10);
 ...
 await page.waitForFunction(()=>document.querySelector('[data-cue="split-values"]').classList.contains('shown'),null,{timeout:5000});
 assert.equal(await page.locator('[data-cue="banana-box"].shown').count(),0);
 await page.waitForFunction(()=>document.querySelector('[data-cue="banana-box"]').classList.contains('shown'),null,{timeout:10000});
 assert.equal(await page.locator('[data-cue="subtract-more-answer"].shown').count(),0);
 await page.waitForTimeout(450);
 await page.locator('#hint-static-box').screenshot({path:'tests/sakura-gengen-banana.png'});
 await page.waitForFunction(()=>document.querySelector('.hunter-sakura-replay').textContent.includes('もういちど'),null,{timeout:15000});
 ...
 await page.evaluate(()=>{
  window.__hintSpeech=[]; window.__hintCancels=0; voiceOn=true;
  Object.defineProperty(window.speechSynthesis,'speak',{configurable:true,value:u=>window.__hintSpeech.push(u.text)});
  Object.defineProperty(window.speechSynthesis,'cancel',{configurable:true,value:()=>{window.__hintCancels++;}});
  startSession([{a:13,b:9,ans:4}],1);
 });
 await page.locator('#hint-btn-sakura').click();
 assert((await page.evaluate(()=>window.__hintSpeech[0])).includes('10からひいて、たすよ。'));
 await page.locator('[data-method="gengen"]').click();
 assert((await page.evaluate(()=>window.__hintSpeech.at(-1))).includes('2回に分けて、引くよ。'));
 assert((await page.evaluate(()=>window.__hintCancels))>=1);
 await page.evaluate(()=>{HunterSakuraLesson.stop();voiceOn=false;});
 console.log('PASS: changing methods cancels old narration and speaks the new explanation');
